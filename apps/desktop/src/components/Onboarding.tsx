...
import type { AudioPermissionStatus, AudioTestResult, InferenceMode, StartSessionRequest } from "../types";
    ...
    <main className="onboarding-shell">
      ...
      <section className="onboarding-stage">
        <div className="onboarding-copy">
          ...
          {step === 2 && (
            <Step title="Choose an audio source" description="Lecture mode keeps distant speech and treats VAD as an annotation, never a hard gate.">
              <div className="choice-grid">
                ...
                <Choice active={draft.audio_source === "system_audio"} icon={<Headphones size={23} />} title="System audio" detail="Browser, local video, or meeting playback" onClick={() => update("audio_source", "system_audio")} />
              </div>
          ...
          {step === 4 && (
            <Step title="Grant audio permission" description={permissionKind === "microphone" ? "macOS requires microphone permission before EchoLingo can listen." : "macOS requires screen/audio capture permission. You still choose a display or app for each capture."}>
              <div className="permission-card"><div><strong>{permissionKind === "microphone" ? "Microphone" : "System audio"}</strong><span>Status: {selectedPermission?.replace("_", " ") ?? "checking"}</span></div><button className="button button--primary" type="button" disabled={selectedPermission === "granted"} onClick={() => void requestPermission()}>{selectedPermission === "granted" ? <><Check size={17} />Granted</> : "Request permission"}</button></div>
              {selectedPermission === "denied" && <p className="field-error">Permission was denied. Open macOS System Settings → Privacy & Security, enable EchoLingo, then return here.</p>}
            </Step>
          )}
          {step === 5 && (
            <Step title="Test your audio" description={draft.audio_source === "microphone" ? `Speak from your expected lecture position using ${selectedDevice?.name ?? "the selected microphone"}.` : "Choose a display or application, play audio, then wait for the meter result."}>
...
function Choice({ active, detail, icon, onClick, title }: { active: boolean; detail: string; icon: ReactNode; onClick: () => void; title: string }) { return <button className={active ? "setup-choice setup-choice--active" : "setup-choice"} type="button" onClick={onClick}>{icon}<strong>{title}</strong><span>{detail}</span>{active && <Check className="choice-check" size={15} weight="bold" />}</button>; }
